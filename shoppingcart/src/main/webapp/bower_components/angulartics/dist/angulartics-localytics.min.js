/**
 * @license Angulartics v0.17.0
 * (c) 2014 Luis Farzati http://luisfarzati.github.io/angulartics
 * Localytics plugin contributed by http://github.com/joehalliwell
 * License: MIT
 */
!function (a) {
    "use strict";
    a.module("angulartics.localytics", ["angulartics"]).config(["$analyticsProvider", function (a) {
        a.settings.trackRelativePath = !0, a.registerPageTrack(function (a) {
            window.localytics && window.localytics.tagScreen(a)
        }), a.registerEventTrack(function (a, b) {
            window.localytics && a && window.localytics.tagEvent(a, b)
        })
    }])
}(angular);